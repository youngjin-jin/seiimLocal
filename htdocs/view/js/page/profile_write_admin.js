...
var profile_write = (function () {
	...
	return {
		...
		get_nationality : function() {
			...
			$.ajax({
				type: 'POST',
				url: '/controller/profile_write_admin.php',
				dataType: 'json',
				cache: false,
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.result) {
						...
						if (response.info.userId) $('#id').val(response.info.userId);
						if (response.info.position) $('#position').val(response.info.position);
						$('#rankLev').val(response.info.level).prop('selected', true);
						if (response.info.phone2) $('#phone2').val(response.info.phone2);
...
$('#form').on('submit', function() {
	...
	$.ajax({
		...
		dataType: 'json',
		cache: false,
		data: { 'module' : 'user_profile_write', 'name' : name, 'pw' : pw, 'position' : position, 'phone2' : phone2 },
		success: function (response) {
			stop_progress();
			//console.log(response);
