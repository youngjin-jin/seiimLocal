...
<script>
	//주소검색 이벤트
	$('#input8_1_value').on('click', function() {
		new daum.Postcode({
			oncomplete: function(data) {			

				if (data.userSelectedType == 'R'){
					$('#input8_1_value').val(data.roadAddress);
				} else {
					$('#input8_1_value').val(data.jibunAddress);
				}
				...
				// 	$('#input8_1_value').val(data.address);
				// }				
				$('#input8_2_value').focus();
			}
		}).open();
	});
</script>
