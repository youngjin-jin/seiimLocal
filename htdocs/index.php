...
if ($_SESSION['userId']) {
	if ($_SESSION['level'] == 100) {
		...
		if ($device == 'pc') {
			setcookie('device', $device, time()+863000, '/');
			header('location: '.DOMAIN.'/view/admin/pc/html/dashboard.php');
		} else {
			$device = 'mobile';
			setcookie('device', $device, time()+863000, '/');
			header('location: '.DOMAIN.'/view/admin/mobile/html/field_list.php');
		}
	}
} else {
	if (strpos(DOMAIN, 'localhost')) {
		...
		setcookie('device', $device, time()+863000, '/');
		setcookie('member_type', 'user', time()+863000, '/');
		header('location: '.DOMAIN.'/view/user/mobile/html/login.php');
	} else {
		if (strpos(DOMAIN, 'portal.saiifedu.com')) {
			setcookie('device', $device, time()+863000, '/');
			setcookie('member_type', 'admin', time()+863000, '/');
