...
if ($_POST['module'] == 'select_delete') {
	...
	$curl_data = array('id'=>$select_list_str);
	
	//echo print_r($curl_data);exit;
	
	$curl = curl_init();
	curl_setopt_array($curl, array(
		CURLOPT_URL => API_URL.'​/ha/delCompany',
...
if ($_POST['module'] == 'add_form') {
	...
	$company_number	 = clean_xss_tags($_POST['company_number']);
	$site			 = clean_xss_tags($_POST['site']);
	$note			 = clean_xss_tags($_POST['note']);
	
	if ($update_id) {
		$curl_url = API_URL.'/ha/editCompany';
		
		$curl_data = array(
			'id'=>$update_id,
			'businessId'=>$company_number,
			'name'=>$company_name
		);
	} else {
		$curl_url = API_URL.'/ha/regCompany';
...
if ($_POST['module'] == 'get_list') {
	...
	$curl_url = API_URL.'/ha/searchCompany?page='.$page;
	
	if ($company_name) $curl_url .= '&name='.urlencode($company_name);
